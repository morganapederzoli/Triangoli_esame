...
void Triangoli::disegnetto (std::vector<std::vector<double>> coordinate,std::vector<std::vector<double>> punti_notevoli){
	char k;
	double ymin = - punti_notevoli[2][2], ymax;
	...
	while(true){
		if(k=='s'){
			...
			if (coordinate[2][1]>=punti_notevoli[2][1])	ymax = coordinate[2][1] + 0.5;
			else ymax = punti_notevoli[1][1]+1; 
			if (ymax < coordinate[1][0]) ymax = coordinate[1][0];

		commandsToTerminal  << "gnuplot -p -e \" set title \'TriangoloPunti\'; "
							<< "set grid; "
							<< "set size ratio -1; "
							<< "set key off; "
							<< "set xrange ["<<ymin<<":" << coordinate[1][0] + 0.5 << "]; "
